...
export function translateBackgroundPosition(value) {
...
export function createProgram(vertexSource, fragmentSource, uniformValues) {
...
export function bindTexture(texture, unit) {
...
class Ripples {
    ...
    loadImage() {
        ...
        const newImageSource =
            ...
            extractUrl(window.getComputedStyle(this.el).backgroundImage);
    ...
    computeTextureBoundaries() {
        ...
        let backgroundAttachment = computedStyle.backgroundAttachment;
        let backgroundPosition = translateBackgroundPosition(
            computedStyle.backgroundPosition
        );

        // Определяем контейнер для фонового изображения
        let container;
    ...
    hideCssBackground() {
        const inlineCss = this.el.style.backgroundImage;
        ...
        // Получаем значение background-image из CSS (учитывая стили из классов и CSS-файлов)
        this.originalCssBackgroundImage = window.getComputedStyle(
            this.el
        ).backgroundImage;

        // Устанавливаем background-image в 'none'
        this.el.style.backgroundImage = 'none';
    }

    restoreCssBackground() {
        if (this.originalInlineCss) {
...
class RipplesPlugin {
    static init(element, option, ...args) {
        if (!config) {
            throw new Error(
